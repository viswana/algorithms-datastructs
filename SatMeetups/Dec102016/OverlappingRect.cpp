...
bool isOverlappingRects(pair<pair<int, int>, pair<int, int> > r1, pair<pair<int, int>, pair<int, int> > r2) {
  if (r1.second.first < r2.first.first || r2.second.first < r1.first.first) return false;
  if (r1.second.second > r2.first.second || r2.second.second > r1.first.second) return false;
  return true; 
}
